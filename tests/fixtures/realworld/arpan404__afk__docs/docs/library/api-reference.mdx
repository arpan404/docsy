...
- **`await list_background_tools(*, thread_id, run_id, include_resolved=False)`** -> `list[dict]`
  Lists persisted deferred tool tickets for a run.
  _Use for:_ Long-running tool monitoring and external worker reconciliation.

- **`await resolve_background_tool(*, thread_id, run_id, ticket_id, output=...)`** -> `None`
  Marks a deferred ticket as completed and stores output for runner polling.
  _Use for:_ External worker completion callbacks.

- **`await fail_background_tool(*, thread_id, run_id, ticket_id, error=...)`** -> `None`
  Marks a deferred ticket as failed with error details.
  _Use for:_ External worker failure propagation.
